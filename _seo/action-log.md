...
- **⚠️ 残作業（1クリック）**: `contact_form_sent` を**キーイベントにマーク**する。
  GA4は初回受信までイベント一覧に出ず（最大24時間）、未受信のイベント名を
  事前登録するUIが現行版に無い。**管理 → データの表示 → イベント** に
  `contact_form_sent` が現れたら行頭の★を押す。次回セッションか週次で実施
- **検証（次回週次 9/6）**: レポートにセッションが計上されているか / キーイベントのマーク完了
