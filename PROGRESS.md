...
- [ ] LINE: リッチメニュー作成 / 営業時間の記載をどうするか（PROGRESS 2026-08-26(2) 参照）
...
| フッターボタン | 色=LINE標準の緑 / テキスト=「問い合わせる」「記事をチェック」 |
...
- **応答時間**: 全曜日 24時間 → **9:00〜19:00**（`COMPANY.contactHours` に合わせた）。不定休なので曜日は絞らない
- **応答メッセージ**: 既定文が「このアカウントでは個別のお問い合わせを受け付けておりません」で
  アカウントの目的と正反対だったため、受付時間外の案内文に差し替え（タイトルも「受付時間外の自動返信」に）
...
- **営業時間**（ビジネスプロフィール）: 事務所 9:30〜15:30 と 受付 9:00〜19:00 の2種類があり、
  さらに不定休で曜日固定と相性が悪いため未設定のまま
- **リッチメニュー**: 未作成。トーク画面下部の常設メニュー（「料金を見る」「制作実績」「問い合わせ」など）
- **SNSパーツ**: IG/FBを載せる枠があるが、`bullcom2656` / `It Support Bullcom` は bullcom.net と共用なので保留
- **お知らせパーツ**: OFF
- フッターボタンの色をサイトのピンクに合わせるか（今は緑のまま）
