...
- [x] GA4 プロパティ作成・設置 → **2026-08-30 完了**（下記 2026-08-30 (7)）。
      キーイベントのマークも 2026-08-31 に完了し、**A-7 は全て完了**。
      イベント一覧に contact_form_sent / first_visit / page_view / session_start が
      bullcom.website ストリームで受信されており、計測が回っていることも確認済み
...
### 2026-09-13: SEO/GEO 週次レポート 第3回（対象 9/6〜9/12）

- 外形 18/18 PASS。記録 `_seo/weekly-log.md`、提出用 `_seo/reports/2026-09-13.md` → Dropbox PDF
- **索引が下層に広がった**: URL検査で /blog・/faq・9/2 記事が登録済み、検索表示 6・初クリック 1（/faq）
- sitemap は2週間「取得できませんでした」のまま。同一書式の bullcom.jp は検出67なので書式ではなく Google 側の遅れ
- **GA4 の Organic Social は Facebook クローラ**（Boardman / Prineville / Luleå = Meta DC）。実人間は週1〜2
- GEO 3エンジン×（GBP 更新から6日、判定 9/27）
- 承認後に実施: **A-11** GA4 タグのクローラ除外（`app/layout.tsx`、7f2246f、デプロイ済み。ヘッドレス Chrome の
  `--user-agent` で検証）/ **A-12** 9/9 記事の索引リクエスト済み＋週次手順に「新記事の索引リクエスト」を追加

### 2026-09-07: GSC「ページにリダイレクトがあります」通知 → 対応不要

- インデックスレポート更新（9/4付）で登録済み 0→1、未登録 5（403×2 / noindex×2 / リダイレクト×1）。
  リダイレクトの対象は `http://bullcom.website/`（A-1 の https 正規化）。403 は www の2件、noindex は旧 WordPress の
  `/wp-admin/setup-config.php` 2件。5件とも非正規URLか旧サイトの残骸で、正規ページは含まれない → 放置で消える
- 詳細表は `_seo/action-log.md`「追記 2026-09-07」。今後同じ通知が来ても対象が正規URLでなければ無視でよい
- GBP の A-10（説明文・サービス5件・商品1件）は**別セッションで投入完了**（コミット 9281616）。
  本セッションから説明文とサービス5件の反映を読み取りで確認。残りはクチコミ依頼（ユーザー作業）のみ。
  編集画面の直URL: `https://www.google.com/local/business/17165689984747186215/editprofile/{info|services}?hl=ja`

### 2026-09-06: SEO/GEO 週次レポート 第2回（対象 8/30〜9/5）

- 外形 **18/18 PASS**。記録は `_seo/weekly-log.md`、提出用は `_seo/reports/2026-09-06.md` →
  `D:\Data\Dropbox\BULLCOM\週報月報\bullcom.website_2026-09-06.pdf`
- **A-8 達成**: トップの前回クロールが 8/22(403) → 8/30 14:51 成功、「登録済み」に転じた。A-7（GA4）も 17セッション計上で検証完了
- **問題**: sitemap が1週間「取得できませんでした」のまま（サイト側は正常）→ 9/2 記事が Google 未検出。
  GEO 3エンジン×で、回答の情報源が Googleマップ「ウェブデザイナー」枠だと判明（共用GBPのカテゴリ問題）
- **承認後に実施**: A-9 = sitemap 削除→再送信＋ /blog と 9/2 記事の索引リクエスト（済）。ライブテストで
  Googlebot は sitemap.xml を 200 で取得できており、サイト側の問題ではないと確定。
  A-10 = GBP を開いたら「ウェブ デザイナー」は登録済み（10件上限）。本当のギャップはクチコミ0件と説明文。
  GBP の編集 iframe は CDP スクショが固まるので書き換えは見送り、文案を action-log に置いてユーザー作業に
- GA4 の United States 9ユーザーはクローラ/自動化の疑い。来週も続くなら除外設定
- 手順メモ: GSC の URL検査は `inspect?...&id=<URL>` の直リンクだと 404。上部の検査ボックスに入力する。
  GEO テストは Perplexity `search?q=` / ChatGPT は composer に execCommand で挿入 / Claude は `claude.ai/new?q=` で貼り付き
...
- **配置**: 「選ばれる理由」と CTA帯の間に `id="blog"` セクション（見出し「BLOG / 制作の現場から」）。
  新着3本のカード ＋「ブログ一覧を見る →」（`btn-ghost`、CVボタンより弱い階層）
- **共通化**: カードを `components/ui/BlogCard.tsx` に切り出し、`/blog` 一覧と同じ見た目を共用。
  見出しレベルは props（一覧=h2 / トップ=セクションh2の下なので h3）
- **データ**: `app/page.tsx` を async 化し `getLatestBlogs(3)`。microCMS未設定・0件・取得失敗時は
  **セクションごと非表示**（`catch` で空配列）にしてビルドを止めない
- 予約公開で記事が増えると、次のデプロイ時に自動で入れ替わる（ビルド時取得の静的サイトなので
  microCMS Webhook → deploy.yml の既存フローで反映される。追加設定不要）

検証: lint ✓ / build ✓ / 本番でカード3件・一覧ボタン・h3×3 を確認（9/2・8/26・8/19の記事）。
デスクトップ表示は Chrome MCP で目視確認。**モバイル幅はスクショが取れなかった**
（Chrome MCP の resize が最大化ウィンドウに効かず innerWidth=1920 のまま。
ヘッドレスは hero の `min-h-svh` で縦長キャプチャ不可）。
グリッドは `/blog` 一覧と同一クラス（`sm:grid-cols-2 lg:grid-cols-3`、390pxで1列）で
あちらは 2026-07-26 に390px検証済みのため、CSS同一性で担保している。
実機で一度見ておくと確実。

### 2026-09-02 (2): microCMS予約分18本すべてをXへ予約投稿

microCMSの予約公開（毎週水9:45・12/30まで18本）に対応するX予約投稿を**全件登録**した。
すべて **公開日と同じ日の10:00・サムネ画像つき**。投稿文は `scripts/x-scheduled-2026.json` に保存。

| 期間 | 件数 |
|---|---|
| 9月（9/2・9/9・9/16・9/23・9/30） | 5 |
| 10月（7・14・21・28） | 4 |
| 11月（4・11・18・25） | 4 |
| 12月（2・9・16・23・30） | 5 |
| **合計** | **18** |

登録後にX側の予約一覧をスクレイプし、**18件すべてが日付・記事IDともmicroCMSの予約と一致**することを確認済み。

#### ⚠️ X操作の重大な注意（ここで大きくハマった）

**1. `computer type` はXのコンポーザーで文字を破壊する**

日本語を打つと「【新【着【新【】【新【着…」のように1文字ずつ分割・重複して入る。
最初の1〜2件はたまたま通ったが、以降は毎回壊れた。

→ **JavaScriptの `document.execCommand('insertText')` で一括挿入する**。これなら正確に入る:
```js
const ta=document.querySelector('[data-testid="tweetTextarea_0"]');
ta.focus(); document.execCommand('selectAll'); document.execCommand('delete');
document.execCommand('insertText', false, TEXT);
```
挿入後は必ず `ta.innerText` と原文を突き合わせて検証すること。

**2. 予約が「登録できたように見えて実際は入っていない」ことがある**

コンポーザーが空に戻り、トーストも出たのに予約一覧に無い、という事象が2回起きた。
壊れた入力を破棄した際に、直前の予約ごと消えたと見られる。
→ **数件ごとに予約一覧（`/compose/post/unsent/scheduled`）で実在を確認する**。

**3. モーダル（`/compose/post`）よりホーム画面のインラインコンポーザーのほうが安定**

モーダルはDOM上に複数のコンポーザーが同時に存在し、どれに書いているか分からなくなる。
`x.com/home` のインラインコンポーザーだけを使うと状態が単純になる。

**4. 日時セレクトと確定ボタンはJSで操作できる**

月/日/年/時/分の5つの `<select>` に nativeSetter + `change` イベントで値を入れれば React に反映される。
`確認する` → `予約設定` のクリックもJSで通る。1件あたりのブラウザ往復が大幅に減る。
（ヘルパー関数を組んで「入力→画像添付→日時設定→確定」を3ステップに短縮した）

**5. 「ポストを予約」ボタンは1回目のクリックで開かないことがある**

開かなければもう一度押す。ヘルパー側でリトライを入れておくと安定する。

#### 投稿文の作り方（今回の様式）

原稿の「## SNS投稿」（IG用・絵文字多め・タグ5個）をXの文体に直したもの。

- `【新着】` ＋ フック1文 → 空行 → 本文2〜3文 → 空行 → 記事URL → 空行 → **タグ3個**
- 絵文字は使わない（bullcom.net のX投稿と様式を揃えた）
- **280字制限はCJK2倍重み**。URLは23字固定。日本語は実質125字が上限で、
  投入前に全17件をスクリプトで検算した（最大268・最小216）

### 2026-09-02: X（@BULLCOM_co）への予約投稿を開始

本日9:45公開の記事「制作会社と連絡が取れなくなったとき」（`b2c-o07gvrme`）を、
Xのネイティブ予約投稿で **9/2（水）10:00** に登録した。design の記事をXに載せるのはこれが初。
（当初12:00で登録 → ユーザー指示で10:00へ変更し、サムネ画像も添付）

- アカウントは bullcom.net と共用の **@BULLCOM_co**。当初Chromeは @Truck_asahi しか
  ログインしておらず、ユーザーにBULLCOMアカウントの追加ログインをしてもらってから登録した
- 投稿文は原稿の「## SNS投稿」をX用に整形（【新着】＋本文＋記事URL＋タグ3個、280字制限内。
  IG用のハッシュタグ5個は3個に間引き、絵文字は外した）
- 予約済み一覧で bullcom.net の日次予約（朝9時台）と並んでいるのを確認。
  **designは水曜10:00枠**（netの9:05〜9:15の後・記事公開9:45の直後）
- **サムネを添付する**: `public/blog-thumbnails/{記事ID}.jpg` を file_upload で添付。
  bullcom.net の予約投稿もサムネ付きなので様式を揃えた
- ⚠️ 編集時の注意: 予約済みポストの編集中に座標クリックでダイアログ外を触ると
  **コンポーザーが閉じて編集内容が破棄される**（実際に一度画像添付が消えた）。
  ボタン類は find で ref を取って操作すること
- 次回以降: 毎週水曜の記事公開に合わせて同形式（本文＋URL＋タグ3個＋サムネ）で登録する。
  9/9・9/16・9/23…の分はまとめて予約登録も可能（各週水曜10:00）

### 2026-08-30 (7): GA4 を新設・設置（A-7）✅

ユーザーのログイン済みChromeでプロパティを作成し、コード設置・デプロイ・動作確認まで実施。

| 項目 | 値 |
|---|---|
| プロパティ | BULLCOM design bullcom.website（アカウント BULLCOM `82410892`） |
| プロパティID | `552101754` |
| ウェブストリーム | bullcom.website / ストリームID `15526312808` |
| 測定ID | **`G-ET4GPWNTYJ`** |
| 設定 | タイムゾーン日本 / 通貨JPY / 業種ビジネス・産業 / 目標=見込み顧客の発掘＋トラフィック分析 |

#### 実装

- `app/layout.tsx` に bullcom.jp と同じ `<Script>` パターンで gtag を設置（TODOコメント解消）
- **フォームCVは明示イベント `contact_form_sent`**。`ContactForm.tsx` の完了表示effect
  （`?sent=1` 検出時）から `gtag('event', 'contact_form_sent')` を送る
  - 当初案の「`?sent=1` の page_view をCV条件にする」は**不採用**。完了表示の直後に
    `history.replaceState` でURLから `?sent=1` を消す作りのため、page_view の
    page_location に `sent=1` が残るかどうかがタイミング依存になる

#### 検証（本番）

- gtag.js のロード・`dataLayer` への config 投入・**`_ga` / `_ga_ET4GPWNTYJ` クッキー生成**を確認
  （= GA4ライブラリがセッションを開始している）
- `/contact?sent=1` を開くと実コードパスで `contact_form_sent` が dataLayer に積まれることを確認

#### 残作業（1クリック・次回セッションか週次で）

`contact_form_sent` を**キーイベントにマーク**する。GA4は初回受信までイベント一覧に出ず
（最大24時間）、未受信のイベント名を事前登録するUIが現行版に無い。
**管理 → データの表示 → イベント** に現れたら行頭の★を押すだけ。
なおプロパティ作成時の目標設定により `close_convert_lead` / `qualify_lead` が
自動でキーイベントになっているが、実データは入らないので無視してよい。

**これで A-1〜A-7 まで全て対応済み。** 週報のGA4項目（セッション/直帰率/新規記事PV）と
月報のCV計測は次回から実データで埋まる（データ反映は最大48時間後から）。
...
最終日曜だが運用開始初日で月報の集計対象が無いため、ユーザー指示で**初回週次（基準値取り）**として実施。
詳細は `_seo/weekly-log.md` の第1回エントリと `_seo/action-log.md` の **A-8**。

#### 判明したこと（3点連鎖）

1. **GSCプロパティ `sc-domain:bullcom.website` が「未確認」のまま放置されていた**。
   READMEの想定（TXTがあるからプロパティは生きているはず）が外れた —
   プロパティは追加だけされて所有権確認が完了していなかった
2. **トップページの前回クロールは 8/22 = 移管4日前**。旧LiteSpeedサーバーの **403** で終わっており、
   Googleの中では「入れないサイト」のまま。移管後の新サイトは一度もクロールされていない
3. 結果として**インデックス登録 0ページ・検索流入 0**。未登録6の内訳（403×4 / noindex×2）も
   すべて旧サーバーの痕跡（レポート最終更新 8/21）

#### 実施したこと

- GSCの**所有権確認を完了**（DNS TXTで自動確認・ワンクリック）
- **sitemap.xml を送信**（直後は「取得できませんでした」＝新規プロパティ典型の保留表示。来週再確認）
- トップページの**インデックス登録をリクエスト**（ユーザー承認済み・優先クロールキュー入り）
- 外形チェック 18/18 PASS を確認し、週次ログ第1回（基準値: 全指標0）を記録

#### 来週（9/6）の答え合わせ

クロール日が8/26以降へ更新 / 登録済み 0→増 / sitemap「成功しました」化 の3点（A-8・A-2）。

### 2026-08-30 (4): 週報・月報を「自動起動なし・カレンダー登録のみ」に切り替え

運用ルール（`D:\Data\Projects\Next\bullcom\_seo\report-operation.md`）が同日中に改定され、
**スケジュールタスクによる自動起動は置かない**方針になったため、こちらも合わせた。

#### やったこと

- スケジュールタスク2件を削除
  （`bullcom-design-seo-weekly-review` / `bullcom-design-seo-monthly-review`。
  SKILL.md は `C:\Users\Owner\.claude\scheduled-tasks\` に残っている）
- Googleカレンダー（`bullcom.office@gmail.com`）に繰り返し予定2件を登録

| 予定 | 日時 | RRULE |
|---|---|---|
| 【BULLCOM design】SEO/GEO 週次レポート（15分） | 毎週日曜 13:00〜13:15 | `FREQ=WEEKLY;BYDAY=SU` |
| 【BULLCOM design】SEO/GEO 月次レビュー（1時間・最終日曜） | 最終日曜 13:00〜14:00 | `FREQ=MONTHLY;BYDAY=-1SU` |

- どちらも**「予定なし（FREE）」扱い**にして他の予定をブロックしない。色は 3（紫系）
- 説明文に 始め方・手順・記録先のフルパス・主軸KW・サイト固有の注意 を全部書いてある。
  セッションに引き継がなくても予定を見れば実施できる
- 初回は 週報 9/6 ・ 月報 9/27。本日 8/30 の13:00は既に過ぎているので開始日を次回からにした

#### 始め方

カレンダーの予定を見て、`D:\Data\Projects\Next\bullcom-design` のセッションで
「**BULLCOM designの週次レポートやって**」「**BULLCOM designの月次レビューやって**」と言う。

#### 時間帯

日曜13:00。他サイトと重ねないため（bullcom.net 9:00 / bullcom.jp 11:00 /
boatkaitori.com 12:00。boatkaitori の月報は1時間なので13:00まで走る）。

**cron で「第N曜日」は表現できない**（`0 13 22-28 * 0` は曜日と日付のAND/OR評価が実装依存）が、
カレンダーの RRULE なら `BYDAY=-1SU` で素直に書ける。この点でもカレンダー運用のほうが向いている。

### 2026-08-30 (3): Cloudflare側のAIクローラ設定を許可へ。外形チェック 18/18 PASS

ユーザーがログイン中のChromeを使い、ダッシュボードで設定を変更した（A-3 完了）。

#### 変えた設定（ゾーン概要ページ右カラム「AI ボットアクセスを管理」）
...
**設定は2つあった**。robots.txt の管理（宣言）だけでなく、
**ネットワーク層でAIトレーニングクローラーを遮断する設定**が別にある。
robots.txt を直しても後者が「すべてのページでブロック」だとクローラーは到達できない。
当サイトに広告は無いので実害は出ていなかったはずだが、方針に合わせて無効化した。

**場所のクセ**: この設定は Security メニューの下ではなく**ゾーン概要ページの右カラム**にある。
`/{account}/{zone}/ai-crawl-control` のようなURLへ直接飛ぶと「Page not found」になる。

#### 結果

配信される robots.txt から Cloudflare Managed のブロックが消え、自前の内容のみになった。

| | 初回（運用開始時） | 現在 |
|---|---|---|
| 外形チェック | 7/17 PASS | **18/18 PASS** ✅ |

初回チェックで見つかった外形の問題は全て解消。残るは **A-7（GA4）** のみで、
これはGA4プロパティの作成というユーザー作業が先。
...
当初はスケジュールタスクで自動起動する形で作ったが、同日中に運用ルールが
**「自動起動なし・カレンダー登録のみ」**へ改定されたため作り直した（下記 2026-08-30 (4)）。
